...
export const lessons = [
  { id: 'feasibility', chapter: '01', kicker: 'CHAPTER 01 · MODEL FEASIBILITY', title: 'Will the model fit?', short: 'Model feasibility', category: 'Foundations', description: 'Learn to turn a model architecture into a memory budget, and understand what that budget can tell you.', intent: 'Architecture → memory → hardware → a better engineering decision.', text: 'Model feasibility weights KV cache hardware capacity GLM Qwen Mistral DeepSeek interactive memory budget context concurrency working examples', icon: 'box', time: '10 min' },
  { id: 'hardware', chapter: '02', kicker: 'CHAPTER 02 · HARDWARE SPEED LIMITS', title: 'How fast can it possibly go?', short: 'Hardware speed limits', category: 'Foundations', description: 'A model that fits still has a speed limit. Learn to read a device as rates and put a floor under the decode step.', intent: 'Rates → traffic per step → a floor under the time it can take.', text: 'Hardware bandwidth roofline decode step memory bound HBM NVLink C2C Grace LPDDR topology GH200 H100 H200 B200 MI300X RTX 5090 batching amortization throughput latency tokens per second arithmetic intensity interactive', icon: 'cpu', time: '12 min' },
  { id: 'reuse', chapter: '03', kicker: 'CHAPTER 03 · PREFILL, DECODE AND REUSE', title: 'Who shares the weights?', short: 'Prefill, decode & reuse', category: 'Foundations', description: 'Prefill and decode are the same model behaving like two different machines. Learn to read arithmetic intensity, and what batching really does to a sparse checkpoint.', intent: 'Reuse → arithmetic intensity → why a sparse model stops being sparse.', text: 'prefill decode arithmetic intensity batching expert reuse MoE routing top-k sparse GLM DeepSeek Qwen active parameters total parameters saturation tokens per step interactive roofline compute bound memory bound', icon: 'workflow', time: '11 min' },
  { id: 'engine', chapter: '04', kicker: 'CHAPTER 04 · ENGINE INTERNALS', title: 'What does the engine actually do?', short: 'Engine internals', category: 'Systems', description: 'Serving is a scheduler and an allocator. Learn how block-based cache works, and why a reported cache hit is not a free prefill.', intent: 'Blocks → alignment → what a cache hit really saves.', text: 'engine internals vLLM scheduler KV cache manager block size allocation prefix caching cache hit rate TTFT recomputation alignment block boundary padding request lifecycle interactive source code', icon: 'workflow', time: '11 min' },
  { id: 'measure', chapter: '05', kicker: 'CHAPTER 05 · MEASUREMENT', title: 'What did you actually measure?', short: 'Measurement', category: 'Systems', description: 'Turn a stream of token timestamps into metrics you can defend: TTFT, TPOT, inter-token latency, and goodput.', intent: 'Timestamps → definitions → a number you can defend.', text: 'benchmarking measurement TTFT TPOT ITL inter token latency goodput throughput percentile SLO boundary stall average AIPerf timestamps interactive', icon: 'chart', time: '10 min' },
  { id: 'profile', chapter: '06', kicker: 'CHAPTER 06 · FINDING THE BOTTLENECK', title: 'Why is it slow?', short: 'Finding the bottleneck', category: 'Systems', description: 'The same symptom has many mechanisms, and they need opposite fixes. Build a differential and run the test that separates them.', intent: 'Symptom → differential → the test that can embarrass you.', text: 'profiling debugging bottleneck critical path queueing synchronization memory pressure hypothesis counter-hypothesis discriminating test timeline trace differential diagnosis interactive', icon: 'compass', time: '9 min' },
  { id: 'parallel', chapter: '07', kicker: 'CHAPTER 07 · PARALLELISM AND PLACEMENT', title: 'How many devices, and why?', short: 'Parallelism & placement', category: 'Systems', description: 'Tensor, data and expert parallelism divide different things and charge differently. Learn what sharding buys and what it costs on the critical path.', intent: 'Capacity → sharding → the communication you just bought.', text: 'parallelism tensor parallel data parallel expert parallel TP DP EP sharding replication KV heads collectives interconnect NVLink topology placement offload locality interactive', icon: 'cpu', time: '11 min' },
  { id: 'quantize', chapter: '08', kicker: 'CHAPTER 08 · QUANTIZATION', title: 'How small can the weights go?', short: 'Quantization', category: 'Systems', description: 'Stored precision, executed precision, and the scales in between. What quantization reliably buys, and what it only appears to.', intent: 'Stored bits → scales → what the kernel actually computes in.', text: 'quantization FP8 FP4 INT8 INT4 BF16 storage format execution format scales group size effective bits per parameter kernel support dequantization accumulator KV cache dtype quality interactive', icon: 'function', time: '10 min' },
  { id: 'speculate', chapter: '09', kicker: 'CHAPTER 09 · SPECULATION', title: 'Should it guess ahead?', short: 'Speculation', category: 'Systems', description: 'Speculative decoding spends idle compute on guesses. Learn the break-even condition, and why the answer changes with load.', intent: 'Spare compute → acceptance chain → the break-even rate.', text: 'speculative decoding draft verify acceptance rate break-even geometric chain draft length speedup concurrency batch load medusa eagle interactive', icon: 'radar', time: '10 min' },
  { id: 'production', chapter: '10', kicker: 'CHAPTER 10 · THE DEPLOYMENT', title: 'What would you actually deploy?', short: 'The deployment', category: 'Systems', description: 'Chain all nine questions into one reviewable design, with its assumptions and failure modes written down.', intent: 'Nine answers → one design → the uncertainties that remain.', text: 'production capstone deployment design admission control observability reproducibility rollout recovery failure domain uncertainty register overload goodput checklist', icon: 'notebook', time: '9 min' },
];
